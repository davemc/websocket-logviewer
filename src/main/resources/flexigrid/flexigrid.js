...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		var g = {
			...
			addData: function (data) { //parse data
				...
				$(t).append(tbody);
				this.addCellProp($('tbody tr td' ,g.bDiv));
				this.addRowProp($('tbody tr',g.bDiv));
			...
			addRow: function(row) 
			{
					...
					$(tbody).prepend(tr);
					this.addCellProp($('td' ,tr));
					//this.addRowProp($('tbody tr',g.bDiv));
					this.addRowProp($(tr));
					tr = null;
			},
			addCellProp: function ( iter ) // eg $('tbody tr td' ,g.bDiv)
			{
			...
			addRowProp: function( iter ) // eg $('tbody tr',g.bDiv)
			{
					iter.each
					(
						function ()
		...
		//add td properties
		g.addCellProp($('tbody tr td' ,g.bDiv));
		
		//add row properties
		g.addRowProp($('tbody tr',g.bDiv));
	...
	$.fn.flexAddRow = function(row) { // function to add data to grid

		return this.each( function() {
				if (this.grid) this.grid.addRow(row);
			});

	};

	$.fn.flexClearRows = function() { // function to clear data from grid

		return this.each( function() {
				if (this.grid) this.grid.clearRows();
			});
